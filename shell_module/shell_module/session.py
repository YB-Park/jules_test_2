...
class ShellSession:
            ...
            try:
                ...
                if proc_check.returncode == 0:
                    ...
                    shell_cmd_list = [
                        ...
                        "-NonInteractive", "-Command", "$OutputEncoding = [System.Text.UTF8Encoding]::new()"
                    ]
    ...
    async def _update_cwd(self):
        ...
        for _ in range(20):
            try:
                ...
                if constants.CWD_MARKER_START in line:
                    ...
                    if constants.CWD_MARKER_END in line_content:
                        parsed_cwd = line_content.split(constants.CWD_MARKER_END, 1)[0].strip()
                        if parsed_cwd and 'ErrorGettingCWD' not in parsed_cwd: self.current_working_directory = parsed_cwd
                        return
                    if line_content: output_buffer = line_content
                    continue
                if constants.CWD_MARKER_END in line and start_marker_found:
                    ...
                    if parsed_cwd and 'ErrorGettingCWD' not in parsed_cwd: self.current_working_directory = parsed_cwd
                    return
                if start_marker_found: output_buffer += ("\n" if output_buffer else "") + line
        ...
        final_cwd = "\n".join(filter(None, output_buffer.splitlines())).strip()
        if final_cwd and 'ErrorGettingCWD' not in final_cwd:
            self.current_working_directory = final_cwd
        elif self.current_working_directory == "~":
    ...
    async def execute_command(self, command: str) -> tuple[str, str, int]:
        ...
        async def read_stream(stream, output_list, stream_name):
            empty_reads = 0; max_empty_reads = 4
            while empty_reads < max_empty_reads:
                try:
                    line_bytes = await asyncio.wait_for(stream.readline(), timeout=0.15)
