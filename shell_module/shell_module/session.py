...
class PowerShellSession(BaseShellSession):
    ...
    async def _read_until_prompt(self, stream=None):
        active_stream = stream or self.process.stdout
        buffer = ""
        while True:
            try:
                char = await asyncio.wait_for(active_stream.read(1), timeout=2.0)
                if not char: break
...
class CmdSession(PosixShellSession): # Inherits from Posix for now, can be specialized
